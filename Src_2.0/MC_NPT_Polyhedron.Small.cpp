...
int main(int argc, char **argv)
{
  ...
  cout<<"****************************************************"<<endl;
  cout<<"For small packing, no cell method is used!"<<endl;
  cout<<"All particles in the central box and their images are checked for ovelap!"<<endl;
  ...
  for(int i=0; i<Param.Get_Nstage(); i++) //the stages...
      ...
      for(int j=0; j<Param.Get_Ncycle()*PK.N; j++) //the cycles...
	{
	  ...
	  res = MC.ParticleMove(Param.Get_p_trans(), Param.Get_TransMod(), Param.Get_RotMod(), index, PK);
      ...
      if(Hnew_Acc > 1) //the step size is too large...
	{
      ...
      MC.BoundaryMove(Param.Get_Ndeform(),Param.Get_p_uphill(),Param.Get_Strain_Rescale(),Param.Get_Global_StrainMod(),Param.Get_Shear_StrainMod(), PK);
