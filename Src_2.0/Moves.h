...
class Moves{
 ...
 public:
  ...
  void UpdateTranslation(double* &); // param: (1) CenerLm

  void GetRotation(double, Polyhedron &); //param:(1) RotMod (2) Particle config
  void UpdateRotation(Polyhedron &);

  int ParticleMove(double, double, double, int, Packing &, Cells &);
  ...
  int ParticleMoveII(double, double, double, int, Packing &, Cells &);
  ...
  int BoundaryMove(int, double, double, double, double, Packing &, Cells &);
